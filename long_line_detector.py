...
* CLAHE-only preprocessing (removes slow Retinex, 4x faster)
* Constrained HoughLinesP (replaces redundant LSD)
...
# ---------- Default config values (can be overridden by YAML) ----------
DEFAULT_CONFIG = {
    "edge_detection": {
    ...
    "tracking": {
        ...
        "alpha": 0.15,  # smoothing factor for exponential
    ...
    def load(
        ...
        if path.exists():
            ...
            if H is not None:
                ...
                mapper = cls(H, H_inv.astype(np.float32), (bev_w, bev_h), use_cuda, None, None, cache_maps)
                if cache_maps:
                    mapper._init_remap_maps(frame_shape)
                return mapper
        # fallback: identity homography
        H = np.eye(3, dtype=np.float32)
        H_inv = np.eye(3, dtype=np.float32)
        mapper = cls(H, H_inv, (w, h), False, None, None, cache_maps)
        if cache_maps:
            mapper._init_remap_maps(frame_shape)
        return mapper

    def _init_remap_maps(self, frame_shape: Tuple[int, int]) -> None:
        """Pre-compute remap maps for 2-3x speedup in warping (vectorized)."""
        h, w = frame_shape[:2]
        # Create coordinate grids
        y_coords, x_coords = np.mgrid[0:h, 0:w].astype(np.float32)
        # Convert to homogeneous coordinates
        ones = np.ones((h, w), dtype=np.float32)
        coords_homogeneous = np.stack([x_coords, y_coords, ones], axis=2).reshape(-1, 3).T
        # Apply homography
        warped_homogeneous = self.H @ coords_homogeneous
        # Normalize and reshape
        warped_homogeneous = warped_homogeneous / (warped_homogeneous[2:3, :] + 1e-6)
        map_x = warped_homogeneous[0, :].reshape(h, w)
        map_y = warped_homogeneous[1, :].reshape(h, w)
        self.map_x = map_x.astype(np.float32)
        self.map_y = map_y.astype(np.float32)

    def warp(self, frame: np.ndarray) -> np.ndarray:
        if self.cache_maps and self.map_x is not None and self.map_y is not None:
    ...
    def step(
        ...
        if measurement is not None:
            ...
            if not self.initialized:
                ...
                self.state = "TRACKING"
            self.confidence = 0.2 + 0.6 * measurement_conf
        else:
            if self.initialized:
                self.state = "LOST"
...
def bilateral_laplacian_edge_detection(
    ...
    laplacian = cv.Laplacian(bilateral, cv.CV_32F, ksize=laplacian_ksize)
    edges = np.abs(laplacian)
    edges = cv.normalize(edges, None, 0, 255, cv.NORM_MINMAX).astype(np.uint8)
    _, edges = cv.threshold(edges, 30, 255, cv.THRESH_BINARY)
    return edges
...
def adaptive_canny_edge_detection(
    ...
    """Adaptive Canny with local mean thresholding (~8ms, robust to lighting)."""
    mean = cv.adaptiveThreshold(
        gray, 255, cv.ADAPTIVE_THRESH_MEAN_C, cv.THRESH_BINARY, block_size, c
    )
    low = np.percentile(mean[mean > 0], 20) if np.any(mean > 0) else 50
    high = np.percentile(mean[mean > 0], 80) if np.any(mean > 0) else 150
    return cv.Canny(gray, int(low), int(high))


# ---------- Preprocessing Methods ----------
def clahe_only(frame: np.ndarray, clip_limit: float = 2.0, tile_grid_size: Tuple[int, int] = (8, 8)) -> np.ndarray:
...
def clahe_unsharp_mask(
    ...
    """CLAHE + Unsharp mask (~4-5ms, for faint lines)."""
    clahe_result = clahe_only(frame, clip_limit, tile_grid_size)
    blurred = cv.GaussianBlur(clahe_result, (0, 0), sigma)
    sharpened = cv.addWeighted(clahe_result, 1.0 + strength, blurred, -strength, 0)
    return sharpened


def contrast_stretch(frame: np.ndarray) -> np.ndarray:
    """Contrast stretching (~1-2ms, fastest but riskiest)."""
    lab = cv.cvtColor(frame, cv.COLOR_BGR2LAB)
    L, a, b = cv.split(lab)
...
def estimate_vanishing_point_ransac(
    ...
    for seg in segments:
        ...
        lines.append((dx/norm, dy/norm, x1, y1))
    ...
    for _ in range(iterations):
        ...
        # Lines: (x,y) = (x1,y1) + t*(dx1,dy1) and (x,y) = (x2,y2) + s*(dx2,dy2)
        # Solve: x1 + t*dx1 = x2 + s*dx2, y1 + t*dy1 = y2 + s*dy2
        ...
        try:
            ...
            for dx, dy, x0, y0 in lines:
                ...
                dist = abs((vp_y - y0) * dx - (vp_x - x0) * dy)
                if dist < threshold:
                    inliers += 1
...
class LinePipeline:
    def __init__(self, bev_shape: Tuple[int, int], config: dict) -> None:
        self.config = config
        if config["tracking"]["method"] == "exponential_smoothing":
            self.tracker = ExponentialSmoothingTracker(alpha=config["tracking"]["alpha"])
        else:
            # Fallback to simple exponential smoothing if Kalman not implemented
            self.tracker = ExponentialSmoothingTracker(alpha=config["tracking"]["alpha"])
    ...
    def detect(
        ...
        roi_mask = make_roi_mask(h, w, roi_cfg["height_pct"] / 100.0, roi_cfg["topw_pct"] / 100.0, 1.0, center_offset)

        # Preprocessing
        if preproc_cfg["method"] == "clahe_only":
            processed = clahe_only(frame_bev, preproc_cfg["clahe_clip_limit"], tuple(preproc_cfg["clahe_tile_grid_size"]))
        elif preproc_cfg["method"] == "clahe_unsharp":
            processed = clahe_unsharp_mask(
                frame_bev,
                preproc_cfg["clahe_clip_limit"],
                tuple(preproc_cfg["clahe_tile_grid_size"]),
                preproc_cfg["unsharp_strength"],
                preproc_cfg["unsharp_sigma"],
            )
        else:  # contrast_stretch
            processed = contrast_stretch(frame_bev)
        ...
        if not early_exit:
            ...
            else:  # learned (placeholder - would load ONNX model)
        ...
        if early_exit and self.last_follow is not None:
            ...
            # Constrained HoughLinesP (angle-aware, replaces LSD)
            masked = cv.bitwise_and(gray_proc, gray_proc, mask=roi_mask)
            
            # Pre-filter for vertical gradients if using constrained_hough
            if line_cfg["method"] == "constrained_hough":
                grad_x = cv.Scharr(masked, cv.CV_32F, 1, 0)
                grad_y = cv.Scharr(masked, cv.CV_32F, 0, 1)
                grad_mag = cv.magnitude(grad_x, grad_y)
                grad_angle = np.arctan2(grad_y, grad_x)
                # Filter for near-vertical gradients
                vertical_mask = np.abs(np.abs(grad_angle) - np.pi/2) < np.radians(20)
                vertical_edges = (grad_mag > np.percentile(grad_mag[vertical_mask], 70) if np.any(vertical_mask) else grad_mag > 0).astype(np.uint8) * 255
                edges = cv.bitwise_and(edges, vertical_edges)
            ...
            lines = cv.HoughLinesP(
                ...
                rho=1,
                theta=np.pi / 180,
                threshold=max(30, line_cfg["vote_threshold"]),
                minLineLength=int(line_cfg["minlen_pct"] / 100.0 * min_dim),
                maxLineGap=int(line_cfg["gap_pct"] / 100.0 * min_dim),
            )
        ...
        if not early_exit:
            if vp_cfg["method"] == "ransac":
                ...
                # Fallback to mean (old method)
                vp_candidate = None
            if vp_candidate is not None:
                self.vp_hint = vp_candidate
        vp_hint = self.vp_hint

        min_len_px = line_cfg["minlen_pct"] / 100.0 * min(h, w)
        merged = merge_collinear_segments(segments, angle_tol_deg=5.0, gap_px=line_cfg["gap_pct"] / 100.0 * min(h, w))
...
class FrameProcessor:
    ...
    def __init__(self, pipeline: LinePipeline, mapper: GroundPlaneMapper, camera_model: CameraModel, 
                 imu_alignment: IMUAlignment, config: dict):
    ...
    def start(self):
        if self.config["performance"]["use_multithreading"]:
    ...
    def _worker(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                # Put result
                self.result_queue.put((undistorted, aligned, bev, edges, follow_result, segments, vp_hint, bev_debug))
            except queue.Empty:
                continue
            except Exception as e:
                print(f"Worker thread error: {e}")
    
    def process_frame(self, frame: np.ndarray, timestamp: float) -> Optional[Tuple]:
        ...
        if self.config["performance"]["use_multithreading"]:
            ...
            bev = self.mapper.warp(aligned)
            edges, follow_result, segments, vp_hint, bev_debug = self.pipeline.detect(bev, timestamp)
            return (undistorted, aligned, bev, edges, follow_result, segments, vp_hint, bev_debug)
...
def detect_and_overlay(
    ...
    if processor is not None:
        ...
        if result is None:
            # Return previous frame or empty
            return frame, None, None, [], np.zeros((100, 100, 3), dtype=np.uint8)
        undistorted, aligned, bev, edges, follow_result, segments, vp_hint, bev_debug = result
    else:
    ...
    roi_mask_bev = make_roi_mask(h_bev, w_bev, roi_cfg["height_pct"] / 100.0, roi_cfg["topw_pct"] / 100.0, 1.0, center_offset)
...
def main() -> None:
    ...
    processor = None
    if config["performance"]["use_multithreading"]:
        processor = FrameProcessor(pipeline, mapper, camera_model, imu_alignment, config)
        processor.start()
    ...
    print(f"Config: Edge={config['edge_detection']['method']}, Preproc={config['preprocessing']['method']}, "
          f"Tracking={config['tracking']['method']}, Multithread={config['performance']['use_multithreading']}")
